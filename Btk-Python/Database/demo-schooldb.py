...
class Student:
    ...
    def getStudentById(id):
        sql = "select * from student where id=%s" #placeholder=%s
        value = (id,)

        Student.mycursor.execute(sql,value)

        try:
            obj =  Student.mycursor.fetchone() #result = obj
    ...
    @staticmethod #dışarıdan alınan listeye göre çoklu güncelleme işlemi
    def updateStudents(liste):
        sql = "update student set studentnumber=%s,name=%s,surname=%s,birthdate=%s,gender=%s where id=%s"
        values = []
        order = [1,2,3,4,5,0] #bize gelen bilgi içerisindeki 0. indexteki eleman studentNumbera karşlık gelsin son eleman id olsun dedik
...
# student = Student.getStudentById(22)    
...
students = Student.getStudentsGender('K')
print(students)

liste = []
for std in students:
    std = list(std) #tupdeden listeye dönüşüm
    std[2] = 'Ms ' + std[2] #2. index isim kolununa karşılık gelir ve kızların başına Ms ön eki getiririz bu işlem ile
    liste.append(std)
